...
from ui.views.inkcore.template_report_tab import TemplateReportTabMixin
...
class TemplateTabMixin(TemplateReportTabMixin):
    """Tab: generar plantilla → cargar foto rellena → extraer → guardar al banco.

    El reporte por página (E5) y la reasignación manual viven en
    TemplateReportTabMixin (clase base), separados para acotar este archivo —
    mismo patrón que WriterTabMixin(WriterPreviewMixin).
    """
